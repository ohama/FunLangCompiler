...
## Current Milestone: v7.0 Imperative Syntax

**Goal:** Expression sequencing (;), loop constructs (while/for), array/hashtable indexing (.[]), if-then without else를 컴파일하여 LangThree의 imperative 프로그래밍 문법을 네이티브 코드로 지원한다

**Target features:**
- Expression sequencing: `e1; e2` → LetPat(WildcardPat, e1, e2) desugar
- Loop constructs: `while cond do body`, `for i = start to/downto stop do body`
- Array/Hashtable indexing: `arr.[i]`, `arr.[i] <- v` → desugar to array_get/set or hashtable_get/set
- If-then without else: `if cond then expr` → If(cond, then, Tuple([])) desugar
...
---
*Last updated: 2026-03-28 after v7.0 milestone started*
