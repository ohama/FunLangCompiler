...
## Current Milestone: v4.0 Type System & Error Handling

**Goal:** ADT/GADT(discriminated unions), Records(mutable fields 포함), Exception handling(setjmp/longjmp)을 컴파일하여 LangThree의 타입 시스템 기능 대부분을 네이티브 코드로 지원

**Target features:**
- ADT/GADT: TypeDecl, Constructor, ConstructorPat, DataValue — 태그 기반 힙 구조체
- Records: RecordExpr, FieldAccess, RecordUpdate, SetField, RecordPat — mutable fields 포함
- Exception handling: ExceptionDecl, Raise, TryWith — setjmp/longjmp 기반 스택 언와인딩
...
- Array/Hashtable 빌트인 — v5.0 이후
- File I/O 빌트인 — v5.0 이후
- printf/sprintf 포맷 문자열 — 복잡도 높음, v5.0 이후
...
- v4 대상 AST 노드 (LangThree frontend):
  - ADT: TypeDecl, ConstructorDecl, GadtConstructorDecl, Constructor, ConstructorPat, DataValue
  - Record: RecordDecl, RecordFieldDecl, RecordExpr, FieldAccess, RecordUpdate, SetField, RecordPat, RecordValue
  - Exception: ExceptionDecl, Raise, TryWith, TExn type
  - MatchCompile.fs: 이미 Constructor/Record 패턴 디컴파일 지원
...
---
| setjmp/longjmp for exceptions | 가장 단순한 C-level 언와인딩, Boehm GC와 호환 | — Pending |
| 태그 기반 ADT 힙 구조체 | {i64 tag, ptr payload} — 균일 표현과 일관 | — Pending |
| Record as named N-field struct | 튜플과 유사하나 필드 이름 → 인덱스 매핑 추가 | — Pending |

---
*Last updated: 2026-03-26 after v4.0 milestone started*
