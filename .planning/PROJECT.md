...
## Current Milestone: v11.0 — Compiler Error Messages

**Goal:** 컴파일러 에러 메시지에 소스 위치(파일:행:열)를 포함하고, 모호한 메시지를 구체화하며, 디버깅 정보를 보존

**Target features:**
- [ ] failWithSpan 헬퍼로 모든 Elaboration 에러에 소스 위치 추가
- [ ] 파서 에러 메시지 보존 (fallback 파싱 시 원래 에러 출력)
- [ ] MLIR 디버그 파일 보존 (에러 시 경로 포함)
- [ ] Record/Field 에러에 사용 가능한 타입/필드 목록 포함
- [ ] 에러 카테고리 구분 (parse/elaborate/compile)

## Current State

v10.0 shipped. 202 E2E tests. 12 Prelude modules. FunLexYacc 컴파일 블로커 해소.
3,983 lines F# (Elaboration.fs), 1,343 lines C (lang_runtime.c), 12 Prelude .fun files.
...
---
*Last updated: 2026-03-31 after v10.0 milestone completed*
