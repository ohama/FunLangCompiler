...
## Current Milestone: v10.0 — FunLexYacc 네이티브 컴파일 지원

**Goal:** FunLexYacc 셀프호스팅을 위한 잔여 블로커 해소 — 멀티파일 import, 포맷 문자열, CLI 인자, 문자열 키 해시테이블, 컴파일러 버그 수정

**Target features:**
- [ ] Hashtable 문자열 키 지원 — C runtime hash/compare 확장
- [ ] `get_args ()` CLI 인자 접근 — @main 시그니처 변경 + C runtime
- [ ] `open "file.fun"` 멀티파일 import — AST 평탄화
- [ ] `sprintf` / `printfn` 포맷 문자열 — C runtime snprintf 위임
- [ ] `let mut` + for-in 클로저 segfault 수정
- [ ] 연속 if 표현식 invalid MLIR 수정

## Current State

v9.0 shipped. 183 E2E tests. 8 Prelude modules. LangThree feature parity through Phase 62.
3,564 lines F# (Elaboration.fs), 1,100 lines C (lang_runtime.c), 8 Prelude .fun files.
...
---
*Last updated: 2026-03-30 after v10.0 milestone started*
