...
## Current Milestone: v3.0 Language Completeness

**Goal:** LangThree 인터프리터가 지원하는 나머지 연산자, 빌트인, 패턴 매칭 기능을 컴파일러에 추가하여 대부분의 LangThree 프로그램이 컴파일 가능하도록 한다.

**Target features:**
- 누락 연산자: Modulo (%), Char 리터럴, PipeRight (|>), Compose (>>, <<)
- 패턴 매칭 확장: when 가드, OrPat, ConstPat(CharConst)
- 빌트인 확장: failwith, string_sub, string_contains, string_to_int, char_to_int, int_to_char
- Range 문법: [1..10], [1..2..10]
