...
- Phase 104 added (retroactive — already shipped in v0.1.4): printf/eprintf/log/logf 빌트인 + `--log`/`-h`/`--help` CLI 플래그.
- Phase 105 added: Type Check Diagnostic CLI Modes (Issue #25) — `--check`/`--show-typecheck`/`--strict-typecheck`/`--diagnostic-annotations` 4개 옵션. 사용자 명시 요청: type error 시 컴파일 중단 (`--strict-typecheck`).
- Phase 106 added: Prelude full type annotations (11개 파일) — FunLang 캐노니컬 syntax 채택.
- Phase 107 added: Strict field disambiguation (Option A) — last-wins fallback 제거, ambiguous field → 컴파일 에러.
- Phase 108 added (retroactive — v0.1.11 포함): Heap-aware Equal/NotEqual coercion — Ptr ↔ I64 mismatch 시 Ptr 로 promote + structural equality.
- Phase 109 added: Duplicate top-level definition warning — 동일 이름 top-level `let` 충돌 경고 (FunLangCompiler#28 funyacc variant 재발 방지, FunLexYacc#2 관련). `--strict-duplicates` 로 error 승격 옵션.
