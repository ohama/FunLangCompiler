...
|----------|----------|--------|---------|
| `src/FunLangCompiler.Compiler/lang_runtime.h` | `LangHashEntryStr`, `LangHashtableStr` structs + 9 function declarations | VERIFIED | Lines 58-79: both structs present; all 9 declarations present (create, get, set, containsKey, remove, keys, trygetvalue, index_get, index_set). 195 lines total. |
| `src/FunLangCompiler.Compiler/lang_runtime.c` | 3 static helpers + 9 public functions | VERIFIED | Lines 447-592: FNV-1a hash, memcmp find, rehash, plus all 9 public functions. 146 lines of new code. `grep` count: 11 occurrences of `_str` function definitions. |
| `src/FunLangCompiler.Compiler/Elaboration.fs` | Key-type dispatch in all hashtable builtins + IndexGet/IndexSet + 2 new builtin arms + 9 entries in both externalFuncs lists | VERIFIED | Ptr dispatch confirmed in set (line 1209), get (1229), IndexGet (1246), IndexSet (1268), containsKey (1287), remove (1314), trygetvalue (1363). Both externalFuncs lists at lines 3377-3385 and 3628-3636 each contain all 9 `_str` entries. `inferCollectionKind` at line 80 recognizes `hashtable_create_str`. |
