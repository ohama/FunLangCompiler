...
|---------------------------------------------------|------------------------------------------------------------------|------------|-------------------------------------------------------------------------------------------|
| `src/LangBackend.Compiler/MlirIR.fs`              | MlirGlobal, ExternalFuncDecl, LlvmCallOp, LlvmCallVoidOp        | ✓ VERIFIED | All four types/cases present; MlirModule has Globals + ExternalFuncs fields (lines 18-86) |
| `src/LangBackend.Compiler/Printer.fs`             | printGlobal, printExternalDecl, LlvmCallOp/Void cases, printModule order | ✓ VERIFIED | All functions present (lines 11-161); printModule emits globals → extern decls → funcs    |
| `src/LangBackend.Compiler/Pipeline.fs`            | Platform-aware gcLinkFlags, -lgc threaded into clang args        | ✓ VERIFIED | `gcLinkFlags` at line 32; macOS path `-L/opt/homebrew/opt/bdw-gc/lib -lgc`; clang invocation at line 86 |
| `src/LangBackend.Compiler/Elaboration.fs`         | GC_init in @main, GC_malloc for closures, print/println handlers, Globals/ExternalFuncs population | ✓ VERIFIED | `LlvmCallVoidOp("@GC_init")` prepended to @main (line 441-446); `LlvmCallOp(@GC_malloc)` in App dispatch (line 406); print/println cases at lines 363-386; externalFuncs at lines 454-458 |
