...
The FunLang interpreter already has a working `Prelude/` directory with corresponding .fun files (`String.fun`, `Hashtable.fun`, `Char.fun`, `StringBuilder.fun`, `List.fun`, `Array.fun`, `Option.fun`, `Result.fun`). These are the canonical reference implementations. FunLangCompiler Phase 35 creates a parallel `Prelude/` directory with adapted versions of these files.

The key mechanism is **prelude concatenation in the CLI**: `Program.fs` is extended to discover a `Prelude/` directory (same 3-stage search as FunLang), read all `.fun` files in order, concatenate their source with the user's source, then parse the combined text. This avoids adding `FileImportDecl` support to the compiler (which would require AST changes). Each test `.flt` file in `tests/compiler/35-*.flt` inlines the relevant module definition because the test runner compiles single files. The prelude-loading in the CLI is validated by an E2E test that passes a bare program relying on prelude symbols.
...
|-----------|----------|---------|--------------|
| Prelude/*.fun | FunLangCompiler root `Prelude/` (new) | Module wrapper .fun source files | Same approach as FunLang Prelude/ — .fun files compiled by FunLangCompiler |
...
| FunLang Prelude/*.fun | Reference implementations | Copy and adapt function names; remove type-checker-specific content |
...
| FunLang function names (optionMap, resultBind) | PRE-07/08 names (map, bind) | PRE-07/08 requirement specifies `Option.map`, `Option.bind` — new names needed |
...
**Naming alignment required:** FunLang uses `optionMap`, `optionBind`, etc., but PRE-07/08 require `map`, `bind`, `defaultValue`, etc. The FunLangCompiler Prelude files must use the short names.
...
- Moving `type Option` definition to a `Core.fun` that loads first (same approach as FunLang's `Core.fun`)
...
- **Mutual recursion in modules:** `LetRecDecl` in the compiler supports single-binding `let rec`. The `and` keyword for mutual recursion would create a multi-binding `LetRecDecl`. Prelude files should not use `let rec f = ... and g = ...`. All FunLang prelude files use only single `let rec`, so this is not an issue.

## Don't Hand-Roll

| Problem | Don't Build | Use Instead | Why |
|---------|-------------|-------------|-----|
| Prelude .fun files | Write from scratch | Adapt from FunLang's `Prelude/*.fun` | FunLang files are tested and correct; only need name adjustments |
| List sort algorithm | New quicksort in .fun | FunLang's insertion sort (`_insert` + `sort`) | Already proven to work; simple to inline |
...
| Option/Result types | Redefine from scratch | FunLang's `type Option 'a = None | Some of 'a` | Same ADT representation — compiler uses tag-based dispatch |

**Key insight:** 90% of Phase 35 is writing .fun files that look like the FunLang prelude. The compiler doesn't need changes (except CLI prelude loading). The only new work is: (1) creating the Prelude/ directory with adapted .fun files, (2) extending Program.fs to load them, and (3) writing E2E tests.

## Common Pitfalls

### Pitfall 1: Open Module is a No-Op
**What goes wrong:** Writing `open List` at the end of List.fun (as in FunLang) causes no problems in tests but also does nothing — `OpenDecl` is silently skipped by the compiler.
...
**Warning signs:** If `open List` were removed from FunLang prelude but FunLangCompiler tests still pass, that confirms the compiler doesn't use it.
...
**How to avoid:** Keep all recursive functions single-parameter (returning lambdas for additional args). All functions in FunLang prelude already follow this pattern.

### Pitfall 5: String.fun concat Uses string_concat_list
**What goes wrong:** `FunLang/Prelude/String.fun` includes `let concat sep lst = string_concat_list sep lst`. If `string_concat_list` is not implemented in the FunLangCompiler compiler (not in Phase 31 builtins), this would fail.
**Why it happens:** The FunLang prelude was written for the full interpreter feature set; not all builtins may be in FunLangCompiler yet.
...
| FunLang function names (`optionMap`) | FunLangCompiler names (`Option.map`) | Phase 35 | Cleaner API matching requirement spec |
...
- `deps/FunLang/Prelude/` — 9 existing .fun prelude files, verified working
- `src/FunLangCompiler.Compiler/Elaboration.fs` — Phase 25 module flattening (lines 3189-3246), `prePassDecls` (3151-3187), FieldAccess desugar (2290-2296)
- `src/FunLangCompiler.Cli/Program.fs` — single-file CLI, no prelude loading yet
- `tests/compiler/25-*.flt` — confirmed module system works (qualified vars, open, types, exceptions)

### Secondary (MEDIUM confidence)
- FunLang Prelude.fs load order logic — basis for CLI prelude loading pattern
- FunLang Eval.fs FileImportDecl handling — confirmed text-concat avoids this complexity

## Metadata

**Confidence breakdown:**
- Standard stack: HIGH — compiler module system verified working via Phase 25 tests; FunLang prelude files are the canonical reference
...
**Valid until:** Stable indefinitely — depends only on Elaboration.fs and FunLang Prelude/ which are stable
