...
This phase adds source location (file:line:col) to all Elaboration error messages in LangBackend. The codebase already has all the infrastructure needed -- the `Span` type is defined in LangThree's `Ast.fs`, every AST node carries a span, and helper functions `formatSpan`, `spanOf`, and `patternSpanOf` already exist. The work is purely mechanical: create a `failWithSpan` helper and convert each `failwithf` site to use it, changing wildcard `_` span matches to named bindings.
...
No new libraries or dependencies are needed. Everything is in LangThree's `Ast` module, already referenced by `LangBackend.Compiler.fsproj`.
...
```
// --- Command: bash -c 'dotnet run --project %S/../../src/LangBackend.Cli/LangBackend.Cli.fsproj -- %S/44-01-error-location-unbound.fun 2>&1; echo $?'
// --- Input:
// --- Output:
...
- `/Users/ohama/vibe-coding/LangBackend/src/LangBackend.Compiler/Elaboration.fs` -- All 22 failwithf/failwith sites examined
