...
타입 정보 없이 AST 구조만으로 코드 생성을 결정하는 heuristic이 실패할 때, 체계적으로 모든 경로를 커버하는 방법. 근본 해결은 타입 정보를 제공하여 heuristic을 secondary로 강등시키는 것.
...
### Step 5: runtime 호출 시 인자 타입 coercion 확인한다

closure uniform ABI (`%arg1: !llvm.ptr`)로 전달된 값이 C runtime 함수의 기대 타입과 다를 수 있다:

```fsharp
// ❌ BAD: index가 Ptr인데 그대로 전달
[LlvmCallOp(result, "@lang_string_char_at", [collPtr; idxVal])]
// → MLIR error: operand type mismatch '!llvm.ptr' != 'i64'

// ✅ GOOD: Ptr → I64 coercion 추가
let (idxI64, idxCoerce) =
    if idxVal.Type = Ptr then
        let v = { Name = freshName env; Type = I64 } in (v, [LlvmPtrToIntOp(v, idxVal)])
    else (idxVal, [])
[LlvmCallOp(result, "@lang_string_char_at", [collPtr; idxI64])]
```

### Step 6: 근본 해결 — 타입 정보를 제공한다

heuristic은 **타입 정보가 없을 때의 fallback**이다. 가능하면 타입 정보를 제공하여 heuristic을 불필요하게 만든다:

```
문제: FunLang 타입체커가 s.[i]를 거부 → annotationMap 전체 소멸
해결: FunLang #15 — 타입체커에 string indexing 지원 추가
결과: annotationMap이 정상 생성 → isStringExpr 첫 체크에서 해결
     → heuristic fallback이 secondary로 강등
```

이 패턴은 일반화할 수 있다: **컴파일러가 지원하는 기능을 타입체커가 거부하면, 파일 전체의 타입 정보를 잃는다.** 컴파일러에 기능을 추가할 때는 항상 타입체커도 함께 확인한다.

## Example

Issue #22의 5단계 수정 과정 (commits: f6982f7 → 2a2c6a6 → c58ab96 → 4bd62dd → 1631bc5):
...
// Phase 4: FunLang 타입체커 수정 (FunLang #15)
// → annotationMap이 정상 생성되어 heuristic fallback이 secondary로
// → 위 Phase 1-3의 모든 수정이 여전히 유효하지만, primary path는 annotationMap

// Phase 5: closure uniform ABI에서 index coercion
// → %arg1: !llvm.ptr로 전달된 index를 lang_string_char_at(ptr, i64)에 맞게 변환
let (idxI64, idxCoerce) =
    if idxVal.Type = Ptr then
        let v = { Name = freshName env; Type = I64 }
        (v, [LlvmPtrToIntOp(v, idxVal)])
    else (idxVal, [])
```
...
- [ ] runtime 호출 시 Ptr → I64 coercion 필요한지 확인
- [ ] FunLang 타입체커가 해당 기능을 지원하는지 확인 (annotationMap 소멸 방지)
- [ ] 1-param, 2-param, let rec, closure capture, variable index 5가지 경로 테스트
