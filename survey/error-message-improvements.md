...
**Date:** 2026-04-01
**Context:** LangThree Span fix 적용 후, 에러 메시지 품질 향상을 위한 전체 현황 및 개선 방안
...
### 1.1 동작하는 것
- **Span 위치 전파**: LangThree v9.1 (Phase 69)에서 `PositionedToken` + `filterPositioned` 추가. FunLangCompiler에서 이를 사용하도록 수정 완료 (2026-04-01). 에러 메시지에 실제 `file:line:col` 표시.
...
**장점:** LangThree/fslit 수정 불필요, 가장 간단
**단점:** 에러 메시지 파싱에 의존, Prelude 내부 에러는 음수 줄 번호 발생 가능

#### 방안 B: lexbuf 시작 위치 조정 (LangThree 활용)
...
#### 방안 A: errorHandler 커스텀 (LangThree 수정)

FsLexYacc는 `Parser.parse_error` 이벤트를 제공. 현재 LangThree에서 이를 활용하지 않음.
...
fslit (`/Users/ohama/vibe-coding/fslit/`)의 Checker.fs에서:
...
| `IndentFilter.fs` | LangThree | PositionedToken, filterPositioned |
| `Program.fs` | LangThree | parseModuleFromString (Span 전파) |
| `Parser.fsy` | LangThree | ruleSpan/symSpan (245개 사용처, unknownSpan 0) |
