...
**Date:** 2026-03-30
**Purpose:** `langbackend-feature-requests.md` (2026-03-28, v8.0 기준) 을 FunLangCompiler v9.0 + FunLang v7.1 네이티브 API 전환 기준으로 재평가

## Context

- FunLang v7.1에서 dot notation이 제거되고, 네이티브 모듈 API가 도입됨
...
- FunLexYacc 소스를 `.NET interop` → `FunLang 네이티브 모듈 API`로 전환하면 대부분의 gap이 해소됨
- 워크어라운드 가이드: `../FunLang/survey/funlexyacc-workaround-guide.md`
...
FunLexYacc `.fun` 소스를 `.NET interop` → `FunLang 네이티브 모듈 API`로 전환.
가이드: `../FunLang/survey/funlexyacc-workaround-guide.md`

- 전환 대상: 위 해결 완료 항목 44개
- 예상 범위: 19개 .fun 파일, ~384 call site
- FunLang 인터프리터에서 동작 검증 가능
...
*Generated: 2026-03-30 — FunLangCompiler v9.0 (183 E2E tests) + FunLang v7.1 기준*
