# FunLexYacc Compilation Gap Status — LangBackend v9.0

**Date:** 2026-03-30
**Purpose:** `langbackend-feature-requests.md` (2026-03-28, v8.0 기준) 을 LangBackend v9.0 + LangThree v7.1 네이티브 API 전환 기준으로 재평가

## Context

- LangThree v7.1에서 dot notation이 제거되고, 네이티브 모듈 API가 도입됨
- LangBackend v9.0에서 StringBuilder, HashSet, Queue, MutableList, String/Char/Array 빌트인, for-in 패턴, 리스트 컴프리헨션, 문자열 슬라이싱 등이 구현됨 (183 E2E 테스트)
...
- 또는 LangBackend에 C runtime `lang_sprintf` 구현 (포맷 문자열 파싱 필요)

**권장:** LangBackend에 sprintf 구현 (C runtime `snprintf` 위임)
...
FunLexYacc 전환 시 영향을 줄 수 있는 LangBackend v9.0의 기존 제약:
...
### Phase A: FunLexYacc 소스 전환 (LangBackend 변경 불필요)
...
- FunLexYacc 전체를 LangBackend로 컴파일
- 생성된 네이티브 바이너리로 funlex/funyacc 테스트 실행

---

*Generated: 2026-03-30 — LangBackend v9.0 (183 E2E tests) + LangThree v7.1 기준*
