# LangThree (인터프리터) vs LangBackend (컴파일러) 비교
...
LangBackend는 LangThree의 Parser/Lexer/AST를 **submodule로 직접 참조**한다.
- `LangBackend.Compiler.fsproj` → `ProjectReference` → `LangThree`
...
| 모듈 | LangThree | LangBackend | 비고 |
|------|-----------|-------------|------|
| Array.fun | O | O | 동일 |
| Char.fun | O | O | 동일 |
| Hashtable.fun | O | O | LangBackend에 `createStr`, `keysStr` 추가 |
| List.fun | O | O | LangBackend에 `sortBy`, `mapi`, `tryFind`, `choose`, `distinctBy` 추가 |
...
없음. LangBackend는 LangThree의 부분집합.

### 함수명 차이 (Option / Result)

| LangThree | LangBackend | 비고 |
|-----------|-------------|------|
| `optionMap` | `map` | LangBackend가 모듈 컨텍스트에서 간결한 이름 사용 |
| `optionBind` | `bind` | |
...
LangBackend에서 보완하면 완전 동일해지는 항목:
...
**핵심:** 문법은 100% 동일. 빌트인도 실질적으로 동일 (LangBackend에 2개 추가). Prelude 래퍼 4개만 추가하면 모듈 수준에서도 완전 동일.
